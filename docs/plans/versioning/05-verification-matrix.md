...
Run only after Phase 03 creates and pushes `v1.15.0-next.0`.

```bash
npx bmad-method install --modules baut --pin baut=v1.15.0-next.0 --tools codex --yes --directory /tmp/baut-next-smoke
```
...
- installs `baut`
- records `channel: pinned`
- records `version: v1.15.0-next.0`
...
- selected version is latest pure semver tag, not `v1.15.0-next.0`
